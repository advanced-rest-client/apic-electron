...
export class ThemePluginsManager {
  ...
  get pluginManager() {
    if (!this.__pluginManager) {
      this.__pluginManager = new PluginManager({
        cwd: process.env.APIC_THEMES,
        pluginsPath: process.env.APIC_THEMES
      });
    }
  ...
  async install(name, version) {
    ...
    info = await this._createThemeInfo(info);
  ...
  async _installPackage(name, version) {
    try {
      ...
      return await this._installLocalPackage(name);
    } catch (_) {
      return await this._installRemotePackage(name, version);
    }
  }
  /**
  ...
  _compareEngines(remoteInfo) {
    ...
    if (!engines || !engines.apic) {
      return true;
    }
    const engine = engines.apic;
   ...
   * @param {String} info.location
   * @param {String} info.mainFile
   * @param {Boolean=} info.isSymlink
   * @return {Object} An object to be stored in theme info file.
   */
  async _createThemeInfo(info) {
    const result = {
      _id: info.name,
      name: info.name,
