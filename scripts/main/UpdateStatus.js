...
import { dialog, nativeImage, ipcMain } from 'electron';
 ...
 */
export class UpdateStatus extends EventEmitter {
  constructor() {
    super();
    this.state = 0;
    this.lastInfoObject = undefined;
    this.checkDebounce = 5000;
   ...
   */
  listen() {
    ipcMain.handle('check-for-update', this._checkUpdateHandler.bind(this));
    ipcMain.handle('install-update', this.installUpdate.bind(this));
  }
  /**
  ...
  start(settings, skipAppUpdate) {
    ...
    if (!skipAppUpdate) {
      setTimeout(() => this.check(), this.checkDebounce);
    }
  }
  /**
